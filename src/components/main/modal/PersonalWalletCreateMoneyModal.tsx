...
interface personalWalletCreateMoneyDto {
    amount: string;
}

const PersonalWalletCreateMoneyModal = ({eventName}: modalProps) => {
    ...
    const closeModal = () => {
        setModalShow((state) => {
            ...
            newState.personalWalletCreateMoney = false;
            return newState;
        })
    }
    ...
    const handleSubmitClick = async () => {
        ...
        const personalWalletCreateMoneyData: personalWalletCreateMoneyDto = {
            amount: amount,
        }
        const formData = new FormData();
        Object.entries(personalWalletCreateMoneyData).map(([k, v]) => {
            formData.append(k, v);
        })
        logContentList.push({
        ...
        await devInstance.post("/wallet/user/money", formData)
            .then((res) => {
                closeModal();
                logContentList.push({
                    type: logTypeConstant.blue,
    ...
    return (
        <>
            {
                modalShow.personalWalletCreateMoney &&
...
export default PersonalWalletCreateMoneyModal;